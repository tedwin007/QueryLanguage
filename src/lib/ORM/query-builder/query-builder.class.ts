...
import {
  VisitedNode,
  VisitedStatement,
} from "../../visitor/visitor.interfaces";
...
export class QueryBuilderClass extends AbstractQueryBuilder<BuildQueryResponse> {
  ...
  constructor(
    ...
    entitiesMetaData: SimpleEntityDefinition[]
  ) {
  ...
  buildQuery(statement: string): BuildQueryResponse {
    ...
    return {
      ...
      getAutoCompleteOptions: () =>
        parser.computeContentAssist(ParserRules.statement, tokens),
      sqlSelectStatement,
    };
  }

  private toSqlQuery(
    statementList: VisitedStatement[],
    sqlSelectStatement = ""
  ): string {
    const firstStatement = statementList[0];
    const entity = this.getEntityName(firstStatement);
    sqlSelectStatement = `${this.SQLSelect} ${entity} ${
      this.SQLWhere
    } ${this.getQueryCondition(firstStatement, entity)}`;

    for (let i = 1; i < statementList.length; i++) {
      const currentItem = statementList[i];
      sqlSelectStatement +=
        " " +
        (this.hasConjunction(statementList, i)
  ...
  private validateValues(
    ...
    if (!values?.image)
      throw new Error(
        "getValues method failed (bad input " + JSON.stringify(values) + ")"
      );
  ...
  private getEntityName(item: VisitedStatement): string {
    ...
    if (!entityName || !(entityName in this.entitiesMetaData))
      throw new Error(
        "getEntityName method failed, " +
          entityName +
          " was not found in the known entities list "
      );
    return entityName;
  }
  ...
  private validateOperator(
    ...
    if (!operatorName)
      throw new Error(
        "getOperatorName failed, please provide a valid operator name"
      );
    ...
    if (!validOperatorsList.includes(item.sign))
      throw new Error(
        "getOperatorName failed, the operator (" +
          operatorName +
          ") is not valid to the give propType (" +
          propName +
          ":" +
          propType +
          ") "
  ...
  private hasConjunction(
    statementList: VisitedStatement[],
    i: number
  ): boolean {
    return (
      statementList.length > i + 1 &&
      statementList.length % 2 !== 0 &&
      i % 2 !== 0
    );
  }
}
