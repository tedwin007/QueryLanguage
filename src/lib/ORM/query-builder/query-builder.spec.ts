...
describe("QueryBuilderClass", () => {
  let ql = new QueryBuilderClass(
    { createQueryBuilder: jasmine.createSpy() } as any,
    [
      {
        name: "Test",
  ...
  describe("Simple Statement", () => {
    ...
    it("BuildQuery should return a valid (simple) query", () => {
      const sqlSelectStatement =
        ql.buildQuery(simpleStatement).sqlSelectStatement;
  ...
  describe("Complex Statement", () => {
    describe("Or", () => {
      ...
      it("buildQuery should return a valid query", () => {
        const sqlSelectStatement =
          ql.buildQuery(complexStatement).sqlSelectStatement;
        const expected =
          "SELECT * FROM Test WHERE Test.prop_one=1 OR Test.prop_two=1";
        expect(sqlSelectStatement).toEqual(expected);
      });
    });
      ...
      describe("Complex Statement", () => {
        ...
        it("buildQuery should return a valid query", () => {
          const sqlSelectStatement =
            ql.buildQuery(complexStatement).sqlSelectStatement;
          const expected =
            "SELECT * FROM Test WHERE Test.prop_one=1 And Test.prop_two=1";
          expect(sqlSelectStatement).toEqual(expected);
        });
        ...
        describe("With two different entities", () => {
          beforeEach(() => {
            complexStatement =
              "(Test prop_one = '1') And (Test prop_two = '1')";
          });
