...
describe('QueryBuilderClass', () => {
  it('buildQuery should return "select * from Test where prop_one=1"', (done) => {
    DataSource.prototype.initialize = jasmine.createSpy().and.returnValue(Promise.resolve({ createQueryBuilder: () => { driver: { }as any } }));
    createDataSource({} as any).catch().then((DS: any) => {
