...
export class QueryBuilderClass<T extends ObjectLiteral> extends Visitor {
  ...
  async buildQuery(statements: VisitedStatement[]) {
    let i = 0;
    const newLocal = `select * from ${statements[i].entity.image} where ${statements[i].prop.image}${[statements[i].operator.image]}${statements[i].values.image} ${statements.length > 1 ? statements[i].conjunctionOpt + '' : ''}`;
