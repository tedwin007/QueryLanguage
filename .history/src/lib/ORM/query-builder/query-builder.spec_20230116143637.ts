...
describe("sdf", () => {
  ...
  describe('Simple Statement', () => {
    beforeEach(async () => {
      DS = await createDataSource(config)
    })
    fit('', function (done) {

      runExample(DS, "(Test prop_one = '1') And (Test prop_two = '1')").then((res) => {
...
function runExample(DS: Promise<DataSource | void>, statement: string) {
  if (DS) {
    const ql = new QueryBuilderClass<Test>(DS as DataSource, Test)
