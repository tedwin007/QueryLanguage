...
export class QueryBuilderClass<T extends ObjectLiteral> extends Visitor {
  ...
  async buildQuery(statements: VisitedStatement[]) {
    let i = 0;
    return await this.dataSource.manager.getRepository(this.entity).query(newLocal_1)
  }

  build(statementList: VisitedStatement[]) {
    let newLocal = `select * from`;
    statementList.forEach((item, i) => {
      newLocal += `${item.entity.image} where ${item.prop.image}${[item.operator.image]}${item.values.image}}`

      // if (statementList.length % 2 !== 0) {
