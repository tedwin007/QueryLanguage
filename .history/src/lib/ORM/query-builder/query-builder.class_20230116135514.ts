...
export class QueryBuilderClass<T extends ObjectLiteral> extends Visitor {
  ...
  private getSqlQuery(statementList: VisitedStatement[]) {
    ...
    statementList.forEach((item, i) => {
      ...
      if (statementList.length < i + 1 && (statementList.length % 2 !== 0) && i && i % 2 !== 0) {
        const prop = item[i + 1].prop;
        const operator = statementList[i + 1].operator;
        const values = statementList[i + 1].values;
        sqlSelectStatement += ` ${prop.image}${operator.image}${values.image}`;
      }
    })
