...
export class QueryBuilderClass<T extends ObjectLiteral> extends Visitor {
  ...
  private getSqlQuery(statementList: VisitedStatement[]) {
    ...
    statementList.forEach((item, i) => {
      if (i === 0) {
        sqlSelectStatement = this.getInitialSelectQuery(item, sqlSelectStatement);
      } else if (this.conjunctionOpt(statementList, i)) {
        sqlSelectStatement += this.getExpandedQuery(statementList[i + 1]);
      }
    })
    console.log('SQL QUERY : ', sqlSelectStatement);
    return sqlSelectStatement
  }

  private getInitialSelectQuery(item: VisitedStatement, sqlSelectStatement: string): string {
  ...
  private conjunctionOpt(statementList: VisitedStatement[], i: number): boolean {
